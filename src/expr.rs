    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            NotEqual(box ref lhs, box ref rhs)
                => write!(f, "{} != {}", lhs, rhs),
            LessThan(box ref lhs, box ref rhs)
                => write!(f, "{} < {}", lhs, rhs),
            GreaterThan(box ref lhs, box ref rhs)
                => write!(f, "{} > {}", lhs, rhs),
            ...
            Record(ref data) => {
                write!(f, "{{ ")?;
                let tmp: Result<Vec<()>, _> = data.iter().map(|&(ref name, box ref e)| write!(f, "{}: {}", name, e)).collect();
                tmp?;
                write!(f, " }}")
            },
            Dot(box ref e, ref label) => write!(f, "{}.{}", e, label),
            Variant(ref label, box ref e, box ref ty)
                => write!(f, "{}::{}({})", ty, label, e),
            Match(box ref e, ref branches) => {
                write!(f, "match {} {{", e)?;
                let tmp: Result<Vec<()>, _> = branches.iter()
                    .map(|&(ref label, ref name, box ref e)| write!(f, "{} {} => {},", label, name, e))
                    .collect();
                tmp?;
                write!(f, "}}")
            },
